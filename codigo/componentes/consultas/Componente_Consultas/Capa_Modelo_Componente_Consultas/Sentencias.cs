...
namespace Capa_Modelo_Componente_Consultas
{
    ...
    // Samuel Estuardo Gómez Lec 0901-21-10616 (estandarización y refactorización)
    public class Sentencias
    {
        ...
        public DataTable fun_EjecutarConsulta(string tabla, string sorden = "")
        {
            DataTable dt = new DataTable();
            try
            {
        ...
        // Consulta con filtro global (busca en todas las columnas)
        public DataTable fun_EjecutarConsultaConFiltro(string tabla, string sfiltro = "", string sorden = "")
        {
            DataTable dt = new DataTable();
            try
            {
                using (OdbcConnection conexion = con.conexion())
                {
                    ...
                    {
                        OdbcCommand cmdCols = new OdbcCommand($"SHOW COLUMNS FROM {tabla};", conexion);
